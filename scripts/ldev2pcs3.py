...
def process_ldev_line(line, all_nodes, all_zpools, all_lustre_services):
    ...
    if "MGS" in lustre_service_name and lustre_service_name != "MGS":
        print(
            f'WARNING: Target name "{lustre_service_name}" is not valid, '
            'using "MGS" instead',
            file=sys.stderr,
        )
    ...
    if zpool in all_zpools:
        ...
        if not set(all_zpools[zpool]).issuperset(set(nodes)):
            print(
                'zpool "{zpool}" already instantiated on ' "incompatible set of nodes",
                file=sys.stderr,
            )
            sys.exit(1)
...
def configure(args, all_nodes, all_zpools, zpool_order, all_lustre_services):
    ...
    for node in all_nodes:
        ...
        run("pcs constraint location " + node + "prefers " + args.mgmt_node, args)
    ...
    for zpool in zpool_order:
        ...
        run("pcs resource meta " + zpool_resource + " migration-threshold=1", args)
    ...
    for service in lustre_service_order:
        ...
        run("pcs constraint order " + zpool_resource + " then " + service, args)
        cmd = (
...
parser.add_argument(
    "--cluster-name",
    help=(
        "Name of the cluster " "(by default guessed from a random hostname in infile)"
    ),
)
parser.add_argument(
    "--fence-ipaddr",
